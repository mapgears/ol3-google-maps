...
ExampleBuilder.prototype.render = async function(dir, chunk) {
  ...
  data.olVersion = pkg.devDependencies.ol;
  data.filename = htmlName;
  data.googleMapsApiKey = process.env.GOOGLE_MAPS_API_KEY;
  ...
  if (data.resources) {
    ...
    for (let i = 0, ii = data.resources.length; i < ii; ++i) {
      ...
      const remoteResource = resource.indexOf('//') === -1 ?
        `https://mapgears.github.io/ol-google-maps/examples/${resource}` : resource;
